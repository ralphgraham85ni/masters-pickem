...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
const STORAGE_KEY = "masters-pickem-picks-v2";
const AUTO_REFRESH_SEC = 60;

// ─── INITIAL DEMO PICKS ───────────────────────────────────────────────────────
const DEFAULT_PICKS = [
  { id: 1, name: "Ralph",  golfers: ["Scottie Scheffler", "Rory McIlroy", "Collin Morikawa", "Tommy Fleetwood"] },
  { id: 2, name: "Paddy",  golfers: ["Ludvig Aberg", "Bryson DeChambeau", "Jon Rahm", "Shane Lowry"] },
  { id: 3, name: "Seamus", golfers: ["Xander Schauffele", "Viktor Hovland", "Patrick Cantlay", "Russell Henley"] },
  { id: 4, name: "Ciara",  golfers: ["Brooks Koepka", "Jordan Spieth", "Justin Thomas", "Cameron Smith"] },
];
...
async function fetchGolferScores(golferNames) {
  ...
  const espnPlayers = competitors.map((c) => {
    ...
    const toParStat = stats.find((s) =>
      s.name === "scoreToPar" || s.abbreviation === "TOT" || s.name === "toPar"
    );
    let score = 0;
    if (toParStat) {
      const raw = toParStat.displayValue;
      score = raw === "E" ? 0 : (parseInt(raw, 10) || 0);
    }

    const missedCut = ["STATUS_MISSED_CUT", "STATUS_WITHDRAWN", "STATUS_DISQUALIFIED"].includes(statusName);
    const position  = c.status?.displayValue ?? "-";
  ...
  for (const name of golferNames) {
    ...
    result[name] = match
      ? { score: match.score, missedCut: match.missedCut, position: match.position }
      : { score: 0, missedCut: false, position: "-" };
  }
  return result;
}
...
if (stored) setPicks(JSON.parse(stored));
  ...
  // Fetch on mount
  useEffect(() => { refreshScores(false); }, []); // eslint-disable-line
  ...
  const friendTotals = picks
    .map((pick) => {
      ...
      const golferDetails = pick.golfers.map((name) => {
        const s = scores[name];
        if (!s) return { name, score: null, missedCut: false, position: "-" };
        hasScores = true;
  ...
  const saveEdit = () => {
    if (editingPick === "new") {
      const newId = Math.max(0, ...picks.map((p) => p.id)) + 1;
      setPicks([...picks, { id: newId, name: editForm.name, golfers: editForm.golfers }]);
    } else {
      setPicks(picks.map((p) => p.id === editingPick ? { ...p, name: editForm.name, golfers: editForm.golfers } : p));
    }
    setEditingPick(null);
  };
  const deletePick = (id) => setPicks(picks.filter((p) => p.id !== id));
  const medals = ["🥇", "🥈", "🥉"];
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .friend-header { display: flex; align-items: center; justify-content: space-between; padding: 14px 18px; background: var(--green-deep); }
        ...
        .golfer-grid { display: grid; grid-template-columns: 1fr 1fr; }
        ...
        .manage-card-name { font-weight: 700; font-size: 15px; }
        ...
        .form-input { width: 100%; padding: 9px 12px; border: 1px solid #ddd; border-radius: 7px; font-size: 14px; font-family: 'Source Sans 3', sans-serif; background: white; }
      ...
      <div className="app">
        <header className="header">
          <div className="header-inner">
            <div className="header-flag">⛳ Augusta National · 2025</div>
        ...
        <main className="main">
          ...
          {view === "leaderboard" && (
            <>
                ...
                <div className="refresh-left">
                  <div className="countdown-wrap" title={"Auto-refresh in " + countdown + "s"}>
                    <svg width="26" height="26" viewBox="0 0 26 26">
                      ...
                      <circle className="ring-fill" cx="13" cy="13" r={r}
                  ...
                  <div className="last-updated">
                    {lastUpdated
                      ...
                      : loading ? "Fetching scores…" : "Loading…"}
                  </div>
                </div>
              ...
              {!scoresLoaded && !loading && <div className="empty-state">Fetching live Masters scores via ESPN…</div>}

              {scoresLoaded && friendTotals.length > 0 && (() => {
          ...
          {view === "manage" && !editingPick && (
            <div className="manage-section">
              <div className="manage-note">
                ✏️ Add friends and their 4 golfer picks here. Scores refresh automatically from ESPN every 60 seconds. Missed cut players receive an automatic <strong>+5 penalty</strong>.
              </div>
              {picks.map((pick) => (
          ...
          <div className="edit-overlay" onClick={(e) => e.target === e.currentTarget && setEditingPick(null)}>
            <div className="edit-modal">
              ...
              <div className="form-actions">
                <button className="btn-cancel" onClick={() => setEditingPick(null)}>Cancel</button>
                <button className="btn-save" onClick={saveEdit}>Save Picks</button>
              </div>
            </div>
          </div>
        )}
